  ...
  switch (method) {
    case "GET":
      try {
        ...
        if (!topic) return res.status(404).json({ msg: "Tool not found" });
    ...
    case "PUT":
      try {
        ...
        if(!topicUpdate) return res.status(404).json({msg: "Tool not found"})
    ...
    case "DELETE":
      try {
        ...
        if(!deletedTopic) return res.status(400).json({msg: "Tool not found"})
